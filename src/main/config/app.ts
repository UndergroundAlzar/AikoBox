...
export async function getAppConfig(force = false): Promise<IAppConfig> {
  if (force || !appConfig) {
    appConfigWriteQueue = appConfigWriteQueue
      .catch(() => {})
      .then(async () => {
        ...
        if (JSON.stringify(mergedConfig) !== JSON.stringify(parsedConfig)) {
          await writeFileAtomically(appConfigPath(), stringify(mergedConfig))
        }
...
export async function patchAppConfig(patch: Partial<IAppConfig>): Promise<void> {
  appConfigWriteQueue = appConfigWriteQueue
    ...
    .then(async () => {
      ...
      const nextConfig = deepMerge(structuredClone(appConfig), structuredClone(patch))
      ...
      await writeFileAtomically(appConfigPath(), stringify(nextConfig))
